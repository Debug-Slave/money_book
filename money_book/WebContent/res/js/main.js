...
function loaderColorCustomInit(){
	document.styleSheets[7].addRule(".loader"+loaderType, 'background-color: blue;');
	document.styleSheets[7].addRule(".loader"+loaderType+":after", 'background-color: rgba(0, 255 ,255 ,.5);');
	document.styleSheets[7].addRule(".loader"+loaderType+":before", 'background-color: rgba(0, 255 ,255 ,.5);');
}

function drawLoaderPanel(targetId) {
	var loaderPanel = "<div class='panel'>"+
									"<div class='panel-body'>"+
										"<div class='col-md-12 col-sm-12' id='loaderPanel' >"+
										"</div>"+
									"</div>"+                    
								"</div>";
	var _$target = $("#"+targetId).html(loaderPanel);
	
	DS_loader.drawLoader("loaderPanel", {typeNumber : loaderType});
}

function clickMenu(url) {
	if(!$(".menu_"+url).hasClass("active")) {
		$(".ripple").removeClass("active");
		$(".menu_"+url).addClass("active");
		drawLoaderPanel("content");
		setTimeout(function() {
			callMenu(url);
		}, 1000);
	}
...
function callMenu(url, param) {
	$.ajax({
		...
		success : function(data){
			$("#content").html(data);
			if(!$("#mimin-mobile").hasClass("reverse")) {
		...
		error : function(request,status,error){
			var html = "<div class='panel'>"+
									"<div class='panel-body'>"+
									"<div class='col-md-12 col-sm-12' style='text-align:center; padding-top: 90px; padding-bottom:90px;'>"+
									"<img style='max-width : 500px;' src='/res/img/"+request.status+".png'>"+
									"</div>"+
								"</div>"+                    
							"</div>";
			$("#content").html(html);
